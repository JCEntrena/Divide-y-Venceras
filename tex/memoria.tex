...
\subsection{Análisis de eficiencia}
El algoritmo produce una única llamada recursiva con un argumento de tamaño como mucho $\frac{n}{2}$, siendo $n$ el tamaño actual. Por tanto tenemos la recurrencia
\begin{equation}
\label{eqmay}
T(n)=\left\lbrace
	    \begin{array}{lr}
...
$$ x_m-x_{m-1}=0 \Rightarrow x_n = 1^n\ c_1 $$
Y hallamos una solución particular para la ecuación en recurrencia no homogénea. Por ejemplo, la sucesión $\{0, 2, 6, 14, 30\dots\}=\{2^{n+1}-2\}_{n\ge 0}$ cumple la ecuación. La solución general es por tanto
$$ x_m = 1^m\ c_1 + 2^{m+1}-2\ \forall m\ge 0$$
Al deshacer el cambio de variable tenemos:
$$ T(n) = c_1 + 2n-2\ \in \mathcal{O}(n)$$
El algoritmo de búsqueda del elemento mayoritario es lineal.
...
\subsection{Algoritmo}
	...
	Dado un array con valores numéricos, queremos calcular la mayor diferencia entre dos elementos a y b, con la condición de que la posición de b sea mayor que la de a, simulando de esta forma una operación de compraventa de acciones en la que queremos obtener la mayor ganancia, con la natural restricción de que hemos de comprar las acciones antes de venderlas. 
	
	Vamos a utilizar un algoritmo recursivo, utilizando la técnica divide y vencerás. 
	...
	\begin{itemize}
		\item Los valores mínimo y máximo están en la primera mitad.
		\item Los valores mínimo y máximo están en la segunda mitad. 
		\item El valor mínimo está en la primera mitad, y el máximo en la segunda. 
	\end{itemize}
	
	Para resolver esto, llamamos recursivamente a la función sobre ambas mitades, y calculamos el mínimo en la primera mitad, y el máximo en la segunda. Comparamos entre los tres resultados para quedarnos con la mayor ganancia y devolvemos las posiciones correspondientes. 
...
$$ x_m-2x_{m-1}=0 \Rightarrow x_n = 2^n\ c_1 $$
Hallamos una solución particular para la ecuación en recurrencia no homogénea, como la sucesión $\{0, 2, 8, 24, 64\dots\}=\{n2^{n}\}_{n\ge 0}$ que verifica la ecuación en recurrencias. La solución general es por tanto:
...
T(n)=\left\lbrace
	    \begin{array}{lr}
            1 & n\le 1\\
            2T\left(\frac{n}{2}\right) + 1 & n>2\\
            \end{array}
	    \right.
\end{equation}
Cambiamos de variable, $m = log_2(n);\ n = 2^m$ y resolvemos la ecuación, lo que nos da una eficiencia $\mathcal{O}(n)$
