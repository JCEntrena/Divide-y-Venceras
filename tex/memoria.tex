	...
	\begin{algorithmic}[1]
		\REQUIRE \ \\
     	...
     	\IF{\texttt{\#elem} <\ 3}
     		...
     		\ENDFOR
    	\ENDIF \\\
    	\RETURN{mayoritario(\texttt{repet})}
	\end{algorithmic}
...
Una vez encontrado el candidato a elemento mayoritario mediante el algoritmo \ref{mayoritario}, es necesario comprobar si realmente lo es (algoritmo \ref{checkmay}), ya que la condición que tenemos es necesaria pero no suficiente. Esta operación se realiza trivialmente en $\mathcal{O}(n)$.
